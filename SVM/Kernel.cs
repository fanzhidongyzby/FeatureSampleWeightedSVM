    ...
    internal abstract class Kernel : IQMatrix
    {
        ...
        public double KernelFunction(int i, int j)
        {
            switch (_kernelType)
            {
                ...
                case KernelType.RBF://对一类来说 这里全部返回得到1 
        ...
        /// <summary>
        /// 为属性进行加权 相似度和样本加权矩阵相乘
        /// </summary>
        public void weightedAttr()
        {
            int num = Problem.attrWeight.Count;//属性个数
            //属性加权
            int sampleNum = _x.Length;//样本个数
            for (int i = 0; i < sampleNum; i++)
            {
                for (int j = 0; j < num; j++)
                {
                    _x[i][j].Value *=Problem.sampleSimilar[i]*Problem.attrWeight[j];
                }
            }
        }
        // 计算内积 欧式距离的平方
